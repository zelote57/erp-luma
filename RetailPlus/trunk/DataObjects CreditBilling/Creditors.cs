...
namespace AceSoft.RetailPlus.Data
{
    ...
    public class Creditors : POSConnection
    {
        ...
        /// <param name="decCredit"></param>
        /// <returns></returns>
        public bool AdjustCredit(Int32 BranchID, string TerminalNo, Int64 TransactionID, Data.ContactDetails clsContactDetails, decimal decCredit, decimal decAmountToReverse)
        {
            bool boRetValue = false;
            try
            {
                ...
                // This will hold the CreditPaymentID Paid by the 
                string strCreditPaymentIDs = "";

                Int64 intContactID = clsContactDetails.ContactID;
                decimal decTotalNotReverse = decAmountToReverse;

                #region tblCreditPaymentCash payment reversal
                
                string SQL = "SELECT * FROM tblCreditPaymentCash " +
                ...
                // 25Feb2015 Reverse all the payment with Reference to the PaymentCash
                foreach (System.Data.DataRow dr in dtT.Rows)
                {
                    decimal decAmount = decimal.TryParse(dr["Amount"].ToString(), out decAmount) ? decAmount : 0;
                    Int64 intCreditPaymentID = Int64.TryParse(dr["CreditPaymentID"].ToString(), out intCreditPaymentID) ? intCreditPaymentID : 0;

                    // revert back the payment
                    SQL = "UPDATE tblCreditPayment SET AmountPaid = AmountPaid - @Amount, AmountPaidCuttOffMonth = AmountPaidCuttOffMonth - @Amount WHERE CreditPaymentID=@CreditPaymentID;";

                    cmd.Parameters.Clear();
                    cmd.Parameters.AddWithValue("@Amount", decAmount);
                    cmd.Parameters.AddWithValue("@CreditPaymentID", intCreditPaymentID);

                    cmd.CommandText = SQL;
                    if (base.ExecuteNonQuery(cmd) > 0)
                    {
                        decTotalNotReverse -= decAmount;
                        // Save the CreditPaymentID of the reversed transaction, so that it won't be automatically reversed in case 
                        // there will be a remaining amount of in decTotalNotReverse
                        strCreditPaymentIDs += (string.IsNullOrEmpty(strCreditPaymentIDs) ? "" : ",") + intCreditPaymentID.ToString();
                    }
                }

                #endregion

                #region tblCreditPaymentCheque payment reversal

                SQL = "SELECT * FROM tblCreditPaymentCheque " +
                              "WHERE TerminalNo=@TerminalNo AND BranchID=@BranchID AND TransactionID=@TransactionID;";
                ...
                // 25Feb2015 Reverse all the payment with Reference to the tblCreditPaymentCheque
                foreach (System.Data.DataRow dr in dtT.Rows)
                {
                    decimal decAmount = decimal.TryParse(dr["Amount"].ToString(), out decAmount) ? decAmount : 0;
                    Int64 intCreditPaymentID = Int64.TryParse(dr["CreditPaymentID"].ToString(), out intCreditPaymentID) ? intCreditPaymentID : 0;
                ...
                if (decTotalNotReverse > 0)
                {
                    decCredit = decCredit - decAmountToReverse + decTotalNotReverse;

                    // pay all the purchases, we will do the reverse to arrive at equal credit in IC_ICC
                    if (string.IsNullOrEmpty(strCreditPaymentIDs))
                        SQL = "UPDATE tblCreditPayment SET AmountPaid = Amount WHERE ContactID=@ContactID;";
                    else
                        SQL = "UPDATE tblCreditPayment SET AmountPaid = Amount WHERE ContactID=@ContactID AND CreditPaymentID NOT IN (" + strCreditPaymentIDs + ");";
                    ...
                    if (string.IsNullOrEmpty(strCreditPaymentIDs))
                        ...
                        SQL = "SELECT * FROM tblCreditPayment WHERE ContactID=@ContactID AND CreditReasonID <> 8 AND CreditPaymentID NOT IN (" + strCreditPaymentIDs  + ") ORDER BY CreditDate DESC;"; //8=deposits
            ...
            ReInsert:
                    if (dtTRowsCount == 0)
                    {
                        #region Create new transaction, coz there is no CreditTransaction
                    ...
                    {
                        #region Reverse existing transactions, ReInsert if there are still remaining

	...
	public class ItemSelectWnd : System.Windows.Forms.Form
	{
		...
		private void LoadOptions()
		{
			...
			else 
			{
				...
				{ 
					dgStyle.GridColumnStyles["BarCode"].Width = 150;
					if (!CONFIG.ShowDescriptionDuringItemSelect)
					{ dgStyle.GridColumnStyles["ProductCode"].Width = this.Width - 290; }
					else
					{ dgStyle.GridColumnStyles["ProductDesc"].Width = this.Width - 290; }
				}
				else
				{
					dgStyle.GridColumnStyles["BarCode"].Width = 0;
		...
		{	
			try
			{
				...
				//{ dt = clsProduct.SearchSaleableDataTable(SearchKey, "ProductCode", SortOption.Ascending, 20, mboShowItemMoreThanZeroQty); }
				//else
				//{ dt = clsProduct.SearchDataTableActiveInactive(ProductListFilterType.ShowActiveAndInactive, SearchKey, "ProductCode", SortOption.Ascending, 20, mboShowItemMoreThanZeroQty); }
				if (mboShowInActiveProducts == false)
				{ dt =  clsProduct.SearchDataTableSimple(ProductListFilterType.ShowActiveOnly, SearchKey, 0, 0, string.Empty, 0, string.Empty, 20, mboShowItemMoreThanZeroQty, true, "ProductCode", SortOption.Ascending); }
				else
				{ dt =  clsProduct.SearchDataTableSimple(ProductListFilterType.ShowActiveAndInactive, SearchKey, 0, 0, string.Empty, 0, string.Empty, 20, mboShowItemMoreThanZeroQty, true, "ProductCode", SortOption.Ascending); }

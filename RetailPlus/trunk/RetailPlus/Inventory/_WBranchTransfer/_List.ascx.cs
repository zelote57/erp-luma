namespace AceSoft.RetailPlus.Inventory._WBranchTransfer
{
	...
	public partial  class __List : System.Web.UI.UserControl
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			if (!IsPostBack)
				if (Visible)
				{
                    ...
                    cboStatus.Items.Add(new ListItem("Show " + WBranchTransferStatus.Open.ToString("G").ToUpper() + " Transfers", WBranchTransferStatus.Open.ToString("d")));
                    cboStatus.Items.Add(new ListItem("Show " + WBranchTransferStatus.ForApproval.ToString("G").ToUpper() + " Transfers", WBranchTransferStatus.ForApproval.ToString("d")));
                    cboStatus.Items.Add(new ListItem("Show " + WBranchTransferStatus.Posted.ToString("G").ToUpper() + " Transfers", WBranchTransferStatus.Posted.ToString("d")));
                    cboStatus.Items.Add(new ListItem("Show " + WBranchTransferStatus.Cancelled.ToString("G").ToUpper() + " Transfers", WBranchTransferStatus.Cancelled.ToString("d"))); 
                    cboStatus.SelectedIndex = cboStatus.Items.IndexOf(cboStatus.Items.FindByValue(WBranchTransferStatus.Open.ToString("d")));
        ...
        protected void lstItem_ItemCommand(object sender, DataListCommandEventArgs e)
        {
            ...
            {
                case "imgItemDelete":
                    stParam = "?task=" + Common.Encrypt("cancel", Session.SessionID) + "&WBranchTransferID=" + Common.Encrypt(chkList.Value, Session.SessionID) + "#cancelsection";
                    Response.Redirect("Default.aspx" + stParam);
                    break;
                case "imgItemEdit":
                    stParam = "?task=" + Common.Encrypt("additem", Session.SessionID) + "&WBranchTransferID=" + Common.Encrypt(chkList.Value, Session.SessionID) + "#itemsection";	
                    Response.Redirect("Default.aspx" + stParam);
                    break;
                case "imgItemPost":
                    stParam = "?task=" + Common.Encrypt("issuegrn", Session.SessionID) + "&WBranchTransferID=" + Common.Encrypt(chkList.Value, Session.SessionID) + "#postsection";	
                    Response.Redirect("Default.aspx" + stParam);
                    break;
		...
		private void Delete()
		{
			if (isChkListSingle() == true)
			{
				...
				{
					Common Common = new Common();
                    string stParam = "?task=" + Common.Encrypt("cancel", Session.SessionID) + "&WBranchTransferID=" + Common.Encrypt(stID, Session.SessionID) + "#cancelsection";	
					Response.Redirect("Default.aspx" + stParam);
				}
			}
			else
		...
		private void Update()
		{
			if (isChkListSingle() == true)
			{
				...
				{
					Common Common = new Common();
					string stParam = "?task=" + Common.Encrypt("additem",Session.SessionID) + "&WBranchTransferID=" + Common.Encrypt(stID,Session.SessionID) + "#itemsection";	
					Response.Redirect("Default.aspx" + stParam);
				}
			}
			else
		...
		private void IssueGRN()
		{
			if (isChkListSingle() == true)
			{
				...
				{
					Common Common = new Common();
					string stParam = "?task=" + Common.Encrypt("issuegrn",Session.SessionID) + "&WBranchTransferID=" + Common.Encrypt(stID,Session.SessionID) + "#postsection";	
					Response.Redirect("Default.aspx" + stParam);
				}
			}
			else
...
//			lblSeparator3.Visible = clsDetails.Write;
		...
		private void LoadSortFieldOptions(DataListItemEventArgs e)
		{
			...
			HyperLink SortByWBranchTransferNo = (HyperLink) e.Item.FindControl("SortByWBranchTransferNo");
			HyperLink SortByWBranchTransferDate = (HyperLink) e.Item.FindControl("SortByWBranchTransferDate");
			...
			HyperLink SortByWBranchTransferSubTotal = (HyperLink) e.Item.FindControl("SortByWBranchTransferSubTotal");

			SortByWBranchTransferNo.NavigateUrl = "Default.aspx" + stParam + "&sortfield=" + Common.Encrypt("WBranchTransferNo", Session.SessionID);
			SortByWBranchTransferDate.NavigateUrl = "Default.aspx" + stParam + "&sortfield=" + Common.Encrypt("WBranchTransferDate", Session.SessionID);
			...
			SortByWBranchTransferSubTotal.NavigateUrl = "Default.aspx" + stParam + "&sortfield=" + Common.Encrypt("SubTotal", Session.SessionID);
		}

...
namespace AceSoft.RetailPlus.Inventory
{
    ...
    public partial class __CloseInventorySubGroup : System.Web.UI.UserControl
	{
        long mlngItemNo = 0;
        ...
        private string CloseInventory()
        {
            ...
            try
            {
                ...
                if (clsERPConfigDetails.PostingDateFrom <= DeliveryDate && clsERPConfigDetails.PostingDateTo >= DeliveryDate)
                {
                    ...
                    clsProductInventories.CloseInventoryByProductSubGroup(int.Parse(cboBranch.SelectedItem.Value), clsAccessUserDetails.UID, DateTime.Parse(txtClosingDate.Text), strReferenceNo, long.Parse(cboProductSubGroup.SelectedItem.Value), cboProductSubGroup.SelectedItem.Text);

                    Products clsProducts = new Products(clsERPConfig.Connection, clsERPConfig.Transaction);
                    clsProducts.LockUnlockForSellingByProductSubGroup(int.Parse(cboBranch.SelectedItem.Value), long.Parse(cboProductSubGroup.SelectedItem.Value), false);
        ...
        private bool ZeroOutActualQuantity()
        {
            ...
            Products clsProduct = new Products();
            boRetValue = clsProduct.ZeroOutActualQuantityByProductSubGroup(int.Parse(cboBranch.SelectedItem.Value), long.Parse(cboProductSubGroup.SelectedItem.Value));
            clsProduct.CommitAndDispose();
            boRetValue = true;
        ...
        private bool CopyPOSToActual()
        {
            ...
            Products clsProduct = new Products();
            boRetValue = clsProduct.CopyPOSToActualByProductSubGroup(int.Parse(cboBranch.SelectedItem.Value), long.Parse(cboProductSubGroup.SelectedItem.Value));
            clsProduct.CommitAndDispose();
            boRetValue = true;
        ...
        private void LockUnlockForSelling(string UnLock = "")
        {
            ...
            clsProducts.LockUnlockForSellingByProductSubGroup(Int32.Parse(cboBranch.SelectedItem.Value), Int64.Parse(cboProductSubGroup.SelectedItem.Value), isLock);
            clsProducts.CommitAndDispose();

            if (!isLock)
            {
                cmdLockUnlockProduct.Text = "Lock Product Sub Group [<label class='ms-error'>Note:</label>Products under this subgroup are <u>CURRENTLY ALLOWED</u> for Selling]";
                cmdLockUnlockProduct.ToolTip = "unlock";
            }
            else
            {
                cmdLockUnlockProduct.Text = "UnLock Product Sub Group [<label class='ms-error'>Note:</label>Products under this subgroup are <u>CURRENTLY NOT ALLOWED</u> for Selling]";
                cmdLockUnlockProduct.ToolTip = "lock";
            }
        }
        #endregion

...
namespace AceSoft.RetailPlus.MasterFiles._ProductSubGroup._Variations
{
	...
	public partial  class __List : System.Web.UI.UserControl
	{
		...
		{
            Response.Redirect(Constants.ROOT_DIRECTORY + "/MasterFiles/_ProductSubGroup/Default.aspx?task=" + Common.Encrypt("list", Session.SessionID));
		}
        ...
        protected void lstItem_ItemCommand(object source, System.Web.UI.WebControls.DataListCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "imgItemDelete":
                    ...
                    clsProductSubGroupVariation.Delete(long.Parse(lblSubGroupID.Text), chkList.Value);
                    clsProductSubGroupVariation.CommitAndDispose();

                    Response.Redirect(lblReferrer.Text);
                    break;
                case "imgItemEdit":
                    stParam = "?task=" + Common.Encrypt("edit", Session.SessionID) + "&subgroupid=" + Common.Encrypt(lblSubGroupID.Text, Session.SessionID) + "&id=" + Common.Encrypt(chkList.Value, Session.SessionID);	
                    Response.Redirect("Default.aspx" + stParam);
                    break;
            }
        }
		...
		{	
			...
			{
				PageData.DataSource = clsDataClass.DataReaderToDataTable(clsProductSubGroupVariation.List(Convert.ToInt64(lblSubGroupID.Text), SortField, sortoption)).DefaultView;
			}
			else
			{						
				string SearchKey = Common.Decrypt((string)Request.QueryString["search"],Session.SessionID);
				PageData.DataSource = clsDataClass.DataReaderToDataTable(clsProductSubGroupVariation.Search(Convert.ToInt64(lblSubGroupID.Text), SearchKey, SortField, sortoption)).DefaultView;
			}
		...
		private bool Delete()
		{
			...
			{
				ProductSubGroupVariations clsProductSubGroupVariation = new ProductSubGroupVariations();
				clsProductSubGroupVariation.Delete(Convert.ToInt16(lblSubGroupID.Text),stIDs.Substring(0,stIDs.Length-1));
				clsProductSubGroupVariation.CommitAndDispose();
			}

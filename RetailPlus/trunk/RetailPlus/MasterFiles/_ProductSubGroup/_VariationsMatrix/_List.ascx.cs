...
namespace AceSoft.RetailPlus.MasterFiles._ProductSubGroup._VariationsMatrix
{
	...
	public partial  class __List : System.Web.UI.UserControl
	{
		...
		{
			Response.Redirect("/RetailPlus/MasterFiles/_ProductSubGroup/Default.aspx?task=" + Common.Encrypt("list",Session.SessionID));
		}
        ...
        protected void lstItem_ItemDataBound(object sender, DataListItemEventArgs e)
		{
			...
			else if(e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
                ...
                lnkVariation.NavigateUrl = "Default.aspx?task=" + Common.Encrypt("details", Session.SessionID) + "&id=" + Common.Encrypt(chkList.Value, Session.SessionID) + "&subgroupid=" + Common.Encrypt(lblSubGroupID.Text, Session.SessionID);

                HyperLink lnkUnitName = (HyperLink)e.Item.FindControl("lnkUnitName");
                lnkUnitName.Text = dr["UnitName"].ToString();
                lnkUnitName.NavigateUrl = Constants.ROOT_DIRECTORY + "/MasterFiles/_Unit/Default.aspx?task=" + Common.Encrypt("details", Session.SessionID) + "&id=" + Common.Encrypt(dr["UnitID"].ToString(), Session.SessionID); 
        ...
        protected void lstItem_ItemCommand(object source, System.Web.UI.WebControls.DataListCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "imgItemDelete":
                    ...
                    clsProductSubGroupVariationsMatrix.Delete(chkList.Value);
                    clsProductSubGroupVariationsMatrix.CommitAndDispose();

                    LoadList();
                    break;
                case "imgItemEdit":
                    stParam = "?task=" + Common.Encrypt("edit", Session.SessionID) + "&id=" + Common.Encrypt(chkList.Value, Session.SessionID) + "&subgroupid=" + Common.Encrypt(lblSubGroupID.Text, Session.SessionID);
                    Response.Redirect("Default.aspx" + stParam);
                    break;
            }
        }
		...
		private void ManageSecurity()
		{
			...
			clsDetails = clsAccessRights.Details(UID,(int) AccessTypes.ProductSubGroupVariations); 
		...
		{	
			...
			{
				PageData.DataSource = clsDataClass.DataReaderToDataTable(clsProductSubGroupVariationsMatrix.BaseList(Convert.ToInt64(lblSubGroupID.Text), SortField, sortoption)).DefaultView;
			}
			else
			{						
				string SearchKey = Common.Decrypt((string)Request.QueryString["search"],Session.SessionID);
				PageData.DataSource = clsDataClass.DataReaderToDataTable(clsProductSubGroupVariationsMatrix.Search(Convert.ToInt64(lblSubGroupID.Text), SearchKey, SortField, sortoption)).DefaultView;
			}
		...
		private bool Delete()
		{
			...
			{
				ProductSubGroupVariationsMatrix clsProductSubGroupVariationsMatrix = new ProductSubGroupVariationsMatrix();
				clsProductSubGroupVariationsMatrix.Delete(stIDs.Substring(0,stIDs.Length-1));
				clsProductSubGroupVariationsMatrix.CommitAndDispose();
			}
		...
		private void Update()
		{
			if (isChkListSingle() == true)
			{
				...
				if (stID!=null)
				{
					string stParam = "?task=" + Common.Encrypt("edit",Session.SessionID) + "&subgroupid=" + Common.Encrypt(lblSubGroupID.Text,Session.SessionID) + "&id=" + Common.Encrypt(stID,Session.SessionID);	
					Response.Redirect("Default.aspx" + stParam);
				}
			}
			else

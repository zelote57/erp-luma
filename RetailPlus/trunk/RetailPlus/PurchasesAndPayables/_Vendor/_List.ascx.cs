	...
	public partial  class __List : System.Web.UI.UserControl
	{
		...
		protected void lstItem_ItemDataBound(object sender, DataListItemEventArgs e)
		{
			...
			else if(e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
				...
				string stParam = "?task=" + Common.Encrypt("details",Session.SessionID) + "&id=" + Common.Encrypt(dr["ContactID"].ToString(),Session.SessionID);

                HyperLink lnkContactCode = (HyperLink)e.Item.FindControl("lnkContactCode");
                lnkContactCode.Text = dr["ContactCode"].ToString();
                lnkContactCode.NavigateUrl = Constants.ROOT_DIRECTORY + "/PurchasesAndPayables/_Vendor/Default.aspx" + stParam;

                HyperLink lnkContactName = (HyperLink)e.Item.FindControl("lnkContactName");
                lnkContactName.Text = dr["ContactName"].ToString();
                lnkContactName.NavigateUrl = Constants.ROOT_DIRECTORY + "/PurchasesAndPayables/_Vendor/Default.aspx" + stParam;

				Label lblContactGroupName = (Label) e.Item.FindControl("lblContactGroupName");
				lblContactGroupName.Text = dr["ContactGroupName"].ToString();
		...
		{	
			...
			{
				PageData.DataSource = clsContact.SuppliersAsDataTable(null,0,SortField, sortoption).DefaultView;
			}
			else
			{						
				string SearchKey = Common.Decrypt((string)Request.QueryString["search"],Session.SessionID);
				PageData.DataSource = clsContact.SuppliersAsDataTable(SearchKey,0,SortField, sortoption).DefaultView;
			}

	...
	public partial  class __List : System.Web.UI.UserControl
	{
        ...
        protected void lstItem_ItemDataBound(object sender, DataListItemEventArgs e)
        {
            ...
            else if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                CreditMemoStatus status = (CreditMemoStatus)Enum.Parse(typeof(CreditMemoStatus), dr["CreditMemoStatus"].ToString());
                if (status == CreditMemoStatus.Posted || status == CreditMemoStatus.Cancelled || lnkReturnNo.Text.Substring(0, Constants.SALES_RETURN_CODE.Length) == Constants.SALES_RETURN_CODE)
                {
        ...
        private void LoadList()
        {
            ...
            {
                PageData.DataSource = clsCreditMemos.ListAsDataTable(SortField, sortoption).DefaultView;
            }
            else
            {
                string SearchKey = Common.Decrypt((string)Request.QueryString["search"], Session.SessionID);
                PageData.DataSource = clsCreditMemos.SearchAsDataTable(SearchKey, SortField, sortoption).DefaultView;
            }

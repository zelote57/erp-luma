...
{
	public class SplitPaymentItemWnd : Form
	{
        ...
        private Data.SalesTransactionItemDetails[] mSalesTransactionItemDetails;
        public Data.SalesTransactionItemDetails[] SalesTransactionItemDetails { set { mSalesTransactionItemDetails = value; } }
		...
		private void InitializeComponent()
		{
            ...
            this.groupBox1.Text = "Click the diner` to pay.";
            // 
            // dgItems
            // 
            ...
            this.Name = "SplitPaymentItemWnd";
            this.ShowInTaskbar = false;
            this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
            this.Load += new System.EventHandler(this.SplitPaymentItemWnd_Load);
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.SplitPaymentItemWnd_KeyDown);
		...
		private void SplitPaymentItemWnd_Load(object sender, System.EventArgs e)
		{
            if (mboIsRefund)
            {
		...
		private void SplitPaymentItemWnd_KeyDown(object sender, KeyEventArgs e)
		{
			switch (e.KeyData)
			{
                case Keys.Up:
        ...
        private void LoadData()
        {
            ...
            decimal AmountDue = 0, OrigAmountDue = 0, DiscountableAmount = 0;
            ...
            for (Int32 iCtr = 1; iCtr < NoOfDiners + 1; iCtr++)
            {
                AmountDue = 0; OrigAmountDue = 0; DiscountableAmount = 0;
                foreach (Data.SalesTransactionItemDetails item in mSalesTransactionItemDetails)
                {
                    if (item.PaxNo == iCtr)
                    {
                        AmountDue += item.Amount;
                        OrigAmountDue += item.Amount;
                        if (item.Discount == 0) DiscountableAmount += item.Amount;
                    }
                }

                if (mclsSalesTransactionDetails.ChargeType == ChargeTypes.FixedValue)
                {
                    AmountDue += mclsSalesTransactionDetails.ChargeAmount * (OrigAmountDue / mclsSalesTransactionDetails.SubTotal);
                }
                ...
                {
                    AmountDue -= mclsSalesTransactionDetails.TransDiscount * (DiscountableAmount / mclsSalesTransactionDetails.DiscountableAmount);
                }
                else if (mclsSalesTransactionDetails.DiscountCode == TerminalDetails.SeniorCitizenDiscountCode)
                {
                    AmountDue -= (DiscountableAmount / (1 + (TerminalDetails.VAT / 100))) * (mclsSalesTransactionDetails.TransDiscount / 100);
                }
                else if (mclsSalesTransactionDetails.DiscountCode == TerminalDetails.PWDDiscountCode)
                {
                    AmountDue -= DiscountableAmount * (mclsSalesTransactionDetails.TransDiscount / 100);
                }
                ...
                ItemDataTable.Rows.Add(iCtr.ToString(), AmountDue.ToString("#,##0.#0"), "0.00", AmountDue.ToString("#,##0.#0"), "0.00");
            }

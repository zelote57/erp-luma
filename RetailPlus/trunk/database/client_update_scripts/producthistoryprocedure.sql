...
BEGIN
	...
	*/
	...
	INSERT INTO tblQtySyncToHistory 
		(select @ProductID AS ProductID, ProductDesc, @ProductQty AS ProductQty, @HistoryQty AS HistoryQty, @MatrixQty AS MatrixQty, @ProductQty-@HistoryQty AS Diff FROM tblProducts WHERE ProductID = @ProductID);
...
***********************************************/
DROP PROCEDURE IF EXISTS ProductQtySyncToHistory
GO

CREATE PROCEDURE ProductQtySyncToHistory()
BEGIN
	...
	*************/
	DROP TABLE IF EXISTS tblQtySyncToHistory;
	CREATE TEMPORARY TABLE tblQtySyncToHistory
	(
	...
	REPEAT
		...
		IF NOT done THEN
			call ProductHistorySummary(prodid);
		END IF;
